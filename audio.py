        ...
        async def consumer():
            ...
            while True:
                ...
                try:
                    ...
                    samples = i32.astype(np.float32) * GAIN
                    # Clamp ke range S32
                    np.clip(samples, -2147483648, 2147483647, out=samples)
                    i32_amp = samples.astype('<i4', copy=False)
                    await ws.send(b"\x02" + i32_amp.tobytes())
                finally:
                    q.task_done()
